...
function initNodesDirective($rootScope, $httpBackend, $compile) {
    ...
    // Init a local scope
    let scope: ng.scope = $rootScope.$new();
    // Create a template
    let tmpl: any = angular.element("<nodes></nodes>");
...
describe("Test the Nodes directive", (): void => {
    ...
    it("initialize the nodes tree properly against mockup backend", function () {
        inject(function ($compile, $rootScope, $http, $httpBackend, $timeout, $document) {
            let tmpl: any = initNodesDirective($rootScope, $httpBackend, $compile);
            // Load example data
            ...
            $timeout(function () {
                ...
                let schemaTreeNode: any = tmpl.children().eq(0).children().eq(0).children().eq(1);
                expect(schemaTreeNode[0].nodeName).to.equal("SCHEMA-TREE", "Schema tree not found.");
                console.log("Schema tree found");
                let schemaTreeRoot: any = schemaTreeNode.children().eq(0).children().eq(1);
                let administrationLabel: any = schemaTreeRoot.children().eq(0).children().eq(0).children().eq(1).children().eq(0).children().eq(0).children().eq(3).children().eq(0);
                ...
                $timeout(function () {
                    ...
                    console.log("Administration tree node found");
                    let asfInputs: any = tmpl.children().eq(0).children().eq(2).children().eq(1);
                    ...
                    let accessTab: any = asfInputs.children().eq(2).children().eq(0).children().eq(2).children().eq(0).children().eq(1).children().eq(0);
                    let writeRightButton: any = accessTab.children().eq(0).children().eq(0).children().eq(1).children().eq(0);
